    ...
    if (req.method === "POST") {
      ...
      if (!signedIn) {
        return res.status(500).json({
          error:
            "Internal Error: Server could not log your request in. Please try again later",
          success: false,
        });
      }
        ...
        if (!userPrevRequest) {
          ...
          if (isSame) {
            return res.status(405).json({
              success: false,
              error:
                "You have already requested for this swap! Please wait for them to contact you.",
            });
          }
          ...
          return res.status(405).json({
            success: false,
            error:
              "You have already requested for a swap! Please wait for them to contact you.",
          });
