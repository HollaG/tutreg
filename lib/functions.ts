...
export const formatTimeElapsed = (date: string) => {
  const timeElapsed = Math.abs(new Date().getTime() - new Date(date).getTime());
...
export const tutregToNUSMods = (url: string) => {
  ...
  for (let moduleCode in holder) {
    const moduleString = Object.keys(holder[moduleCode])
      .map((lessonType) => `${lessonType}:${holder[moduleCode][lessonType]}`)
      .join(",");
...
export const getModuleColorWithShade = (
  ...
  const color = COLOR_INDEX_WITH_SHADE[index % COLOR_INDEX_WITH_SHADE.length];
